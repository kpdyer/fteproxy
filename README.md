...
ABSTRACT
--------
Format-Transforming Encryption (FTE) is a paradigm for building cryptographic primitives that enable the user to have fine-grained control of ciphertexts, by way of a programmable input "format." Given a key, plaintext and format it's the job of an FTE scheme to produce ciphertexts that satisfy the constraints specified by the input format. As an initial proof-of-concept we present an FTE scheme capable of accepting a format specified by a regular expression. We bootstrap this scheme to construct an FTE-powered proxy that tunnels arbitrary TCP streams, by only transmitting strings that are in the language generated by the input regular expression.

Intuitively, a regular-language based FTE proxy is well positioned to resist regular-expression based deep-packet inspection (DPI.) An FTE proxy can coerce a DPI system into misclassifying traffic as a target protocol of its choosing. As an example, one can specify an input regular expression that characterizes HTTP traffic, then use the FTE proxy to tunnel Tor traffic. In our paper [1] we show that misclassification (e.g., making Tor "look like" HTTP) can be achieved low overhead and, with judiciously chosen regular expression, can fool many modern DPI systems.

For technical information about FTE, please see our paper [1].

QUICKSTART - Tor Bundle
----------

Obtain the correct Tor bundle for your system from: https://kpdyer.com/fte/

1. Extract the tar file
$ tar zxvf tor-browser-gnu-PLATFORM-ARCH-TOR_VERSION+[fte_relay-VERSION].tar.gz

2. Start the Tor browser bundle
$ cd tor-browser_LOCAL && ./start-tor-browser 


QUICKSTART - fte_relay 
----------

Obtain the correct binary for your system from: https://kpdyer.com/fte/
...
3. On the client, start the FTE relay:  
$ fte_relay --mode client

4. On the client, test that everything works:  
...
$ fte_relay --mode client --client_ip CLIENT_IP --client_port CLIENT_PORT  --server_ip SERVER_IP --server_port SERVER_PORT &

Request that the background FTE client process terminates cleanly:  
$ fte_relay --mode client --stop

Start the FTE server as a background process:  
$ fte_relay --mode server --server_ip SERVER_IP --client_port SERVER_PORT --proxy_ip PROXY_IP --proxy_port PROXY_PORT &

Request that the background FTE server process terminates cleanly:  
$ fte_relay --mode server --stop
